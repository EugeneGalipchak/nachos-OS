...
void poop(int numParam)
{
   //runs the program
   currentThread->space->InitRegisters(); //set the inital register values
...
ExceptionHandler(ExceptionType which)
{
    ...
    if (which == NoException)
      ...
      printf("Page Fault Exception. Not valid in memory. Kill\n");
      exitInternal(-1);
    }
    else if (which == ReadOnlyException) {
      printf("Ready Only Exception. Cannot access to value. Kill\n");
      exitInternal(-1);
    }
    else if (which == BusErrorException) {
      printf("Buss Error Exception. Memory access error. Kill\n");
      exitInternal(-1);
    }
    else if (which == AddressErrorException) {
      printf("Address Error Exception. Unreferable memory address. Kill\n");
      exitInternal(-1);
    }
    else if (which == OverflowException) {
      printf("Over flow Exception. Overflow on arithmetic. Kill\n");
      exitInternal(-1);
    } 
    else if (which == IllegalInstrException) {
      printf("now");
      printf("Illegal Instruction Exception. Cannot process your instruction. Kill\n");
      exitInternal(-1);
    }
      ...
      // for part 8 - this is the Yield part
      else {
