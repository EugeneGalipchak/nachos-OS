...
Thread::Thread(char* threadName , int joinB)
{
    this -> join = joinB;
    go = 0;
    joinLock = new Lock("join Lock");
    joinCon = new Condition("child con");

    joinConPar = new Condition("parent con");
...
Thread::Finish ()
{
    if (this -> join ==0) {
        ...
        joinLock -> Acquire();
        while(!go) {
            joinCon -> Signal(joinLock);
            joinCon -> Wait(joinLock);
            joinCon -> Signal(joinLock);
        }
        joinCon -> Wait(joinLock);
        joinLock -> Release();
...
Thread::Join () {
    ...
    joinLock -> Acquire();
    go = 1;
    //join not call on this thread before
    ASSERT(this -> joinAlreadyCalled == false);

    //assert thread not calling join on itself
    ASSERT(this != currentThread);

    //assert thread object is joinable
    ASSERT(this -> join !=0);

    // join is already called at this poing
    joinAlreadyCalled = true;

    // signal the lock and wait 
    joinCon -> Signal (joinLock);
